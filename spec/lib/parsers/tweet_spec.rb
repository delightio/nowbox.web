...
describe Parsers::Tweet do
  ...
  context "when passed valid json" do
    it "returns a mention object" do
      Parsers::Tweet.parse(@valid_json).class.should == Aji::Mention
    end

    context "when a block returning true is passed" do
      it "returns a mention object" do
        m = Parsers::Tweet.parse @valid_json do |tweet|
          true
        end
  ...
  context "when invalid json is passed" do
    it "returns nil" do
      Parsers::Tweet.parse(@invalid_json).should be_nil
    end
  end

  context "when passed a parsed json hash" do
    it "returns a mention object" do
      Parsers::Tweet.parse(MultiJson.decode(@valid_json)).class.
                           should == Aji::Mention
    end

    it "creates author object if missing" do
      expect { Parsers::Tweet.parse(MultiJson.decode(@valid_json)) }.to
